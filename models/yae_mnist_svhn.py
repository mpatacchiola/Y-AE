...
class Autoencoder:
    def __init__(self, batch_size, channels=1, conv_filters=64, style_size=512, content_size=2, ksize=(5,5), start_iteration=0, dir_header="./"):
        ...
        weight_initializer_implicit = tf.random_normal_initializer(mean=0.0, stddev=0.01) #None
        ...
        with tf.variable_scope("Training"):
            ...
            ##SUPERVISED-LOSSES
            self.loss_supervised_left = tf.losses.softmax_cross_entropy(onehot_labels=self.left_code_content_deterministic, logits=self.code_content_logits)
            self.accuracy_supervised_left = tf.reduce_mean(tf.cast(tf.math.equal(self.labels_placeholder, self.left_code_content_argmax), dtype=tf.float32))
            self.loss_supervised_right = tf.losses.softmax_cross_entropy(onehot_labels=self.right_code_content_random, logits=self.right_code_content_logits)
            ...
            self.loss = self.loss_reconstruction_left + self.loss_supervised_left + tf.multiply(self.lambda_e, self.loss_supervised_right) + tf.multiply(self.lambda_i, self.loss_style_consistency)
            #self.loss = self.loss_reconstruction_left + self.loss_supervised_left + tf.multiply(1.0, self.loss_style_consistency) 
        ...
        with tf.variable_scope("Summaries"):
            ...
            tf.summary.scalar("accuracy_supervised_left", self.accuracy_supervised_left, family="accuracies")
    ...
    def test(self, sess, input_features, input_labels, lambda_e, lambda_i):
        ...
        '''
        loss, loss_r, loss_c, acc_c, loss_e, loss_i  = sess.run([self.loss, self.loss_reconstruction_left, self.loss_supervised_left, self.accuracy_supervised_left, self.loss_supervised_right, self.loss_style_consistency], feed_dict={self.x: input_features, self.labels_placeholder: input_labels, self.lambda_e: lambda_e, self.lambda_i: lambda_i})
